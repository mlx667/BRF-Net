#src/models/res34_swin_ms.py
from .basic_module import *   # 取代 from basic_module import *
...
class Res34_Swin_MS(nn.Module):  ###  low resolution + Multi-scale
    def __init__(self, image_size, block, layers, num_classes, in_channels=3,zero_init_residual=False, groups=1, width_per_group=64):
...
@register_model("res34_swin_ms")
def build_res34_swin_ms(image_size: int = None, num_class: int = 1, **kwargs):
    # 兼容字段名：num_classes / out_channels
    if "num_classes" in kwargs and (num_class == 1 or num_class is None):
        num_class = int(kwargs["num_classes"])
    if "out_channels" in kwargs and (num_class == 1 or num_class is None):
        num_class = int(kwargs["out_channels"])

    if image_size is None:
        if "image_size" in kwargs:
            image_size = int(kwargs["image_size"])
        else:
            raise ValueError("build_res34_swin_ms requires image_size (arg or kwargs['image_size']).")
    in_channels = int(kwargs.get("in_channels", kwargs.get("in_chans", 3)))
